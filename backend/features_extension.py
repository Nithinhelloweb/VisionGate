...
def record_login_attempt(
    ...
    """Persist a login attempt into login_attempts_log."""
    try:
        cursor.execute(
            """
            INSERT INTO login_attempts_log (username, ip_address, success, reason, attempted_at)
            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
            """,
            (username or "unknown", ip_address or "127.0.0.1", bool(success), reason),
        )
    except Exception as e:
        print(f"[LOGIN_ATTEMPT_LOG_ERROR] Could not log attempt: {e}")
...
def create_active_session(
    ...
    device_info: Optional[str] = "Flutter Client",
) -> str:
    """Create a new active session record in active_sessions table and return session_id."""
    sess_id = str(uuid.uuid4())
    try:
        cursor.execute(
            """
            INSERT INTO active_sessions (session_id, reg_no, username, role, ip_address, user_agent, device_info, created_at, last_activity, is_active)
            VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, TRUE)
            """,
            (
                sess_id,
                reg_no or username or "unknown",
                username or reg_no or "unknown",
                role or "user",
                ip_address or "127.0.0.1",
                user_agent or "",
                device_info or "Flutter Client",
            ),
        )
    except Exception as e:
        print(f"[ACTIVE_SESSION_ERROR] Could not create active session: {e}")
    return sess_id
...
async def get_active_sessions(request: Request, limit: int = 100):
    ...
    admin_reg = (admin_user.get("reg_no") or "").lower()
    client_ip = get_client_ip(request)

    cursor.execute(
        """
        SELECT session_id, reg_no, username, role, ip_address, user_agent, device_info, created_at, last_activity, is_active
    ...
    for r in rows:
        ...
        reg_no = str(r[1] or "")
        ip = str(r[4] or "")
        is_curr = (reg_no.lower() == admin_reg and ip == client_ip)
        sessions.append({
            "id": sess_id,
            "session_id": sess_id,
            "reg_no": reg_no,
            "username": r[2] or reg_no,
            "role": r[3] or "user",
            "ip_address": ip,
            "user_agent": r[5] or "",
            "device_info": r[6] or "Mobile / Desktop App",
            "created_at": str(r[7]) if r[7] else "",
            "last_activity": str(r[8]) if r[8] else "",
            "is_active": bool(r[9]),
            "is_current": is_curr,
        })
...
@feature_router.post("/admin/security/sessions/{session_id}/terminate")
async def revoke_active_session(session_id: str, request: Request):
    ...
    return {"success": True, "message": f"Session {session_id} revoked successfully"}


@feature_router.post("/admin/security/sessions/terminate-all-except-current")
async def terminate_all_except_current(request: Request, body: Dict[str, Any] = Body(default={})):
    """Terminate all active sessions except the current session — Admin only."""
    user = require_admin(request)
    current_sess_id = str(body.get("current_session_id") or "").strip()
    client_ip = get_client_ip(request)

    if current_sess_id:
        cursor.execute("UPDATE active_sessions SET is_active = FALSE WHERE is_active = TRUE AND session_id != ?", (current_sess_id,))
    else:
        # Fallback: preserve sessions matching user and client IP
        cursor.execute("UPDATE active_sessions SET is_active = FALSE WHERE is_active = TRUE AND (LOWER(reg_no) != LOWER(?) OR ip_address != ?)", (user.get("reg_no", ""), client_ip))

    write_audit_log(
        action_type="TERMINATE_ALL_OTHER_SESSIONS",
        actor_reg_no=user.get("reg_no"),
        actor_name=user.get("name"),
        actor_role=user.get("role"),
        entity_type="session",
...
async def get_login_attempts_log(request: Request, limit: int = 100):
    """Retrieve security login attempts log with statistics — Admin only."""
    require_admin(request)
    cursor.execute(
        """
    ...
    attempts = [
        {
            ...
            "ip_address": r[2] or "—",
            "success": bool(r[3]),
            "reason": r[4] or ("Success" if r[3] else "Failed login"),
            "attempted_at": str(r[5]) if r[5] else "",
        }
        for r in rows
    ]

    # Calculate 24h summary metrics
    cursor.execute("SELECT COUNT(*) FROM login_attempts_log WHERE success = FALSE AND attempted_at >= CURRENT_TIMESTAMP - INTERVAL '24 hours'")
    r_fail = cursor.fetchone()
    failed_24h = r_fail[0] if r_fail else 0

    cursor.execute("SELECT COUNT(*) FROM login_attempts_log WHERE success = TRUE AND attempted_at >= CURRENT_TIMESTAMP - INTERVAL '24 hours'")
    r_succ = cursor.fetchone()
    success_24h = r_succ[0] if r_succ else 0
...
@feature_router.get("/admin/security/2fa/status")
@feature_router.get("/2fa/status")
async def get_2fa_status(request: Request):
    """Check whether 2FA is enabled for the authenticated user."""
    user = get_current_user_context(request)
    reg_no = user.get("reg_no", "")
    cursor.execute("SELECT is_enabled, enabled_at FROM totp_secrets WHERE LOWER(reg_no) = LOWER(?)", (reg_no,))
    row = cursor.fetchone()
    is_enabled = bool(row[0]) if row and row[0] is not None else False
    enabled_at = str(row[1]) if row and row[1] else None
    ...
    return {
        ...
        "username": user.get("username", reg_no),
        "reg_no": reg_no,
...
@feature_router.post("/2fa/setup")
async def setup_totp_2fa(request: Request):
    """Generate a TOTP secret, provisioning URI, and scannable QR code PNG for 2FA enrollment."""
    user = get_current_user_context(request)
    reg_no = user.get("reg_no", "")
    ...
    account_label = user.get("username") or reg_no
    provisioning_uri = totp.provisioning_uri(name=account_label, issuer_name="Attenda")
    ...
    try:
        ...
        print(f"[2FA] QR Code image generation error: {qr_err}")
    ...
    return {
        ...
        "qr_code": qr_base64,
        "message": "Scan the QR code into Google Authenticator or Microsoft Authenticator, then verify with the 6-digit code.",
    }


@feature_router.post("/admin/security/2fa/verify")
@feature_router.post("/2fa/verify")
async def verify_and_enable_2fa(request: Request, body: Dict[str, Any] = Body(...)):
    ...
    if not totp.verify(code, valid_window=1):
        raise HTTPException(status_code=400, detail="Invalid 6-digit verification code. Please check your authenticator clock and try again.")
    ...
    return {"success": True, "message": "Two-Factor Authentication successfully enabled and verified"}


@feature_router.post("/admin/security/2fa/test")
@feature_router.post("/2fa/test")
async def test_2fa_code(request: Request, body: Dict[str, Any] = Body(...)):
    """Live verification of a 6-digit TOTP code for an already enabled account."""
    user = get_current_user_context(request)
    reg_no = user.get("reg_no", "")
    code = str(body.get("code", "")).strip()
    ...
    cursor.execute("SELECT secret_key, is_enabled FROM totp_secrets WHERE LOWER(reg_no) = LOWER(?)", (reg_no,))
    row = cursor.fetchone()
    if not row or not row[0]:
        raise HTTPException(status_code=400, detail="No 2FA configured for this account.")
    ...
    return {
        ...
        "is_valid": valid,
        "is_enabled": is_enabled,
        "message": "Valid 6-digit code verified successfully" if valid else "Invalid code. Please check authenticator clock.",
    }


@feature_router.post("/admin/security/2fa/disable")
@feature_router.post("/2fa/disable")
async def disable_2fa(request: Request, body: Dict[str, Any] = Body(default={})):
    ...
    if not row or not row[0] or not row[1]:
        return {"success": True, "message": "2FA was not active"}
    ...
    cursor.execute("UPDATE totp_secrets SET is_enabled = FALSE, enabled_at = NULL WHERE LOWER(reg_no) = LOWER(?)", (reg_no,))
    write_audit_log(
        ...
        actor_role=user.get("role"),
        entity_type="security",
        details=f"Disabled Two-Factor Authentication for {reg_no}",
        ip_address=get_client_ip(request),
    )
