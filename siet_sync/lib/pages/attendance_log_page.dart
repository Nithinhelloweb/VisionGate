...
class _AttendanceLogTabState extends State<AttendanceLogTab> {
  ...
  void _mergeAttendanceLogs(Map<String, dynamic> target, Map<String, dynamic> incoming) {
    // 1. Extract times from incoming
    final inCand = incoming['first_half_in_time'] ?? incoming['in_time'] ?? incoming['check_in_time'];
    final outCand = incoming['second_half_out_time'] ?? incoming['out_time'] ?? incoming['check_out_time'];

    final inTs = incoming['timestamp']?.toString() ?? incoming['date']?.toString() ?? '';
    String? inTimeOnly;
    if (inTs.contains('T')) {
      inTimeOnly = inTs.split('T').last.split('.').first;
    } else if (inTs.contains(' ')) {
      inTimeOnly = inTs.split(' ').last.split('.').first;
    }
    final inPunchType = (incoming['punch_type'] ?? incoming['status'] ?? '').toString().toLowerCase();

    String? candIn = inCand?.toString();
    String? candOut = outCand?.toString();

    if (inTimeOnly != null && !inTimeOnly.startsWith('00:00') && inTimeOnly.contains(':')) {
      if (inPunchType == 'check_out' || inPunchType == 'out' || inTimeOnly.compareTo('13:00') >= 0) {
        candOut ??= inTimeOnly;
      } else {
        candIn ??= inTimeOnly;
      }
    ...
    // Merge in_time (earliest)
    final curIn = target['first_half_in_time'] ?? target['in_time'];
    if (curIn == null && candIn != null) {
      target['first_half_in_time'] = candIn;
      target['in_time'] = candIn;
    } else if (curIn != null && candIn != null) {
      final sCur = curIn.toString();
      target['first_half_in_time'] = sCur.compareTo(candIn) <= 0 ? sCur : candIn;
      target['in_time'] = target['first_half_in_time'];
    }

    // Merge out_time (latest)
    final curOut = target['second_half_out_time'] ?? target['out_time'];
    if (curOut == null && candOut != null) {
      target['second_half_out_time'] = candOut;
      target['out_time'] = candOut;
    } else if (curOut != null && candOut != null) {
      final sCur = curOut.toString();
      target['second_half_out_time'] = sCur.compareTo(candOut) >= 0 ? sCur : candOut;
      target['out_time'] = target['second_half_out_time'];
    }

    if (incoming['first_half_out_time'] != null) {
      target['first_half_out_time'] = incoming['first_half_out_time'];
    }
    if (incoming['second_half_in_time'] != null) {
      target['second_half_in_time'] = incoming['second_half_in_time'];
    }

    // 2. Status upgrade
    final s1 = (target['status'] ?? '').toString();
    final s2 = (incoming['status'] ?? '').toString();
    if (s2 == 'Present' || s2 == 'OD' || s2 == 'Leave' || s2.contains('Half')) {
      if (s1 != 'OD' && s1 != 'Leave') {
        target['status'] = s2;
      }
    } else if ((s1 == 'check_in' || s1 == 'check_out' || s1.isEmpty) && (s2 == 'check_in' || s2 == 'check_out')) {
      target['status'] = 'Present';
    }

    if (incoming['first_half_status'] == 'Present') target['first_half_status'] = 'Present';
    if (incoming['second_half_status'] == 'Present') target['second_half_status'] = 'Present';

    // If both in_time and out_time exist, mark status as Present and punch_type as both
    if (target['first_half_in_time'] != null && target['second_half_out_time'] != null) {
      target['punch_type'] = 'both';
      if (target['status'] != 'OD' && target['status'] != 'Leave') {
        target['first_half_status'] = 'Present';
        target['second_half_status'] = 'Present';
        target['status'] = 'Present';
        target['attendance_value'] = 1.0;
      }
    }

    // 3. Timestamp (prefer earlier morning timestamp)
    final t1 = (target['timestamp'] ?? target['date'] ?? '').toString();
    final t2 = (incoming['timestamp'] ?? incoming['date'] ?? '').toString();
    if (t1.contains('00:00:00') && !t2.contains('00:00:00')) {
      target['timestamp'] = t2;
    } else if (!t1.contains('00:00:00') && !t2.contains('00:00:00') && t2.compareTo(t1) < 0) {
      target['timestamp'] = t2;
    }

    // 4. Attendance value
    final v1 = (target['attendance_value'] as num?)?.toDouble() ?? 0.0;
    final v2 = (incoming['attendance_value'] as num?)?.toDouble() ?? 0.0;
    target['attendance_value'] = v1 > v2 ? v1 : v2;

    if (incoming['source'] == 'face_scan') target['source'] = 'face_scan';
  }
  ...
  void _processRawLogs(List<dynamic> rawLogs) {
    ...
    for (var log in rawLogs) {
      ...
      final regNo = (mapLog['reg_no'] ?? '').toString().toLowerCase().trim();
      final ts = (mapLog['timestamp'] ?? mapLog['date'] ?? '').toString();
      final dateOnly = ts.length >= 10 ? ts.substring(0, 10) : ts;

      // Group strictly by reg_no + date so each staff gets exactly 1 unified card per day
      final key = regNo.isNotEmpty && dateOnly.isNotEmpty
          ? '${regNo}_$dateOnly'
          : (mapLog['id'] != null ? 'id_${mapLog['id']}' : 'ts_$ts');
      
      if (!uniqueMap.containsKey(key)) {
        uniqueMap[key] = mapLog;
      } else {
        _mergeAttendanceLogs(uniqueMap[key] as Map<String, dynamic>, mapLog);
      }
    }
  ...
  Widget _buildFaceScanDetails(dynamic log, String displayTime, bool isDark, Color primaryColor, Color cardBg) {
    final rawIn = log['first_half_in_time']?.toString() ?? log['in_time']?.toString() ?? log['check_in_time']?.toString();
    final rawOut = log['second_half_out_time']?.toString() ?? log['out_time']?.toString() ?? log['check_out_time']?.toString();
    
    final formattedIn = _formatTime(rawIn);
    final formattedOut = _formatTime(rawOut);

    String timingText = 'Timestamp: $displayTime';
    if (formattedIn != '--:--' && formattedOut != '--:--') {
      timingText = 'Check-in: $formattedIn • Check-out: $formattedOut';
    } else if (formattedIn != '--:--') {
      timingText = 'Check-in: $formattedIn';
    } else if (formattedOut != '--:--') {
      timingText = 'Check-out: $formattedOut';
    }
  ...
  Widget _buildHalfDayDetails(dynamic log, bool isDark, Color cardBg, Color primaryColor) {
    ...
    if (ts != null && ts.contains('T')) {
      defaultInTime = ts.split('T').last.split('.').first;
    } else if (ts != null && ts.contains(' ')) {
      defaultInTime = ts.split(' ').last.split('.').first;
    }

    final isDefaultMorning = defaultInTime != null &&
        !defaultInTime.startsWith('00:00') &&
        (int.tryParse(defaultInTime.split(':').first) ?? 12) < 13;

    final isDefaultEvening = defaultInTime != null &&
        !defaultInTime.startsWith('00:00') &&
        (int.tryParse(defaultInTime.split(':').first) ?? 0) >= 13;

    final String? morningIn = log['first_half_in_time']?.toString() ??
        log['in_time']?.toString() ??
        log['check_in_time']?.toString() ??
        (isDefaultMorning ? defaultInTime : null);

    final String? morningOut = log['first_half_out_time']?.toString();

    final String? eveningIn = log['second_half_in_time']?.toString();

    final String? eveningOut = log['second_half_out_time']?.toString() ??
        log['out_time']?.toString() ??
        log['check_out_time']?.toString() ??
        (isDefaultEvening ? defaultInTime : null);

    final fhIn = _formatTime(fhStatus == 'Present' ? morningIn : null);
    final fhOut = _formatTime(fhStatus == 'Present' ? morningOut : null);
    final shIn = _formatTime(shStatus == 'Present' ? eveningIn : null);
    final shOut = _formatTime(shStatus == 'Present' ? eveningOut : null);
