...
class _StaffDashboardTabState extends State<StaffDashboardTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final rawRecentAttendance = (data?['recent_attendance'] as List?) ?? [];
    final Map<String, dynamic> uniqueRecentMap = {};
    for (var item in rawRecentAttendance) {
      if (item is! Map) continue;
      final m = Map<String, dynamic>.from(item);
      final reg = (m['reg_no'] ?? '').toString().toLowerCase();
      final ts = (m['timestamp'] ?? '').toString();
      final dateOnly = ts.length >= 10 ? ts.substring(0, 10) : ts;
      final key = reg.isNotEmpty && dateOnly.isNotEmpty ? '${reg}_$dateOnly' : 'id_${m['id'] ?? ts}';
      if (!uniqueRecentMap.containsKey(key)) {
        uniqueRecentMap[key] = m;
      } else {
        final existing = uniqueRecentMap[key] as Map<String, dynamic>;
        final p1 = existing['punch_type']?.toString().toLowerCase();
        final p2 = m['punch_type']?.toString().toLowerCase();
        if ((p1 == 'check_in' && p2 == 'check_out') || (p1 == 'check_out' && p2 == 'check_in')) {
          existing['punch_type'] = 'both';
          existing['status'] = 'Present';
        }
      }
    }
    final recentAttendance = uniqueRecentMap.values.toList();
