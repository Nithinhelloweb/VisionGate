...
class FeaturesService {
  ...
  static Future<List<Map<String, dynamic>>> getActiveSessions(String token) async {
    try {
      ...
      if (res.statusCode == 200) {
        final data = json.decode(res.body);
        final raw = data['sessions'] ?? data['active_sessions'] ?? [];
  ...
  static Future<bool> terminateSession(String token, dynamic sessionId) async {
    try {
      final idStr = sessionId.toString();
      final res = await apiClient.post(
        '$_baseUrl/admin/security/sessions/$idStr/terminate',
        token: token,
      );
      return res.statusCode == 200;
    } catch (_) {
  ...
  static Future<bool> terminateAllExceptCurrent(String token, {String? currentSessionId}) async {
    try {
      final res = await apiClient.post(
        '$_baseUrl/admin/security/sessions/terminate-all-except-current',
        token: token,
        body: json.encode({'current_session_id': currentSessionId ?? ''}),
      );
      return res.statusCode == 200;
    } catch (_) {
      return false;
  ...
  static Future<Map<String, dynamic>> getLoginAttemptsData(String token, {int limit = 50}) async {
    try {
      final res = await apiClient.get(
        '$_baseUrl/admin/security/login-attempts?limit=$limit',
        token: token,
      ...
      if (res.statusCode == 200) {
        ...
        return {
          ...
          'success_24h': data['success_24h'] ?? 0,
        };
      }
    } catch (_) {}
    return {'attempts': <Map<String, dynamic>>[], 'failed_24h': 0, 'success_24h': 0};
  }
  ...
  static Future<Map<String, dynamic>> get2FAStatus(String token) async {
    try {
      final res = await apiClient.get('$_baseUrl/admin/security/2fa/status', token: token);
      if (res.statusCode == 200) {
        return json.decode(res.body);
      }
    } catch (_) {}
  ...
  static Future<Map<String, dynamic>> setup2FA(String token) async {
    try {
      final res = await apiClient.post('$_baseUrl/admin/security/2fa/setup', token: token);
      if (res.statusCode == 200) {
        return json.decode(res.body);
      }
    } catch (_) {}
  ...
  static Future<bool> verify2FA(String token, String code) async {
    try {
      final res = await apiClient.post(
        '$_baseUrl/admin/security/2fa/verify',
        token: token,
  ...
  static Future<Map<String, dynamic>> test2FACode(String token, String code) async {
    try {
      final res = await apiClient.post(
        '$_baseUrl/admin/security/2fa/test',
        token: token,
        body: json.encode({'code': code}),
      );
      if (res.statusCode == 200) {
  ...
  static Future<bool> disable2FA(String token, String code) async {
    try {
      final res = await apiClient.post(
        '$_baseUrl/admin/security/2fa/disable',
        token: token,
        body: json.encode({'code': code}),
      );
      return res.statusCode == 200 && json.decode(res.body)['success'] == true;
